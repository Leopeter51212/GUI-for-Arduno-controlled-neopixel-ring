...
void loop() {
  ...
  while(Serial.available()>0)
  {
    ...
    var = Serial.read();
    if(var == '0'){
      colorWipe(strip.Color(  0,   0,   0,  255), DELAY_TIME);   // True white (not RGB white)      
    }
    if(var== '1'){
    ...
    if(var=='4'){
      on_off(strip.Color(  0,   0,  0,  255), strip.numPixels() - 1, DELAY_TIME);           // turn all LED on
    }
    if(var=='5'){
      on_off(strip.Color(  0,  0,  0,  0), strip.numPixels() - 1, DELAY_TIME);             // turn all LED off
    }
    if(var=='6'){
    ...
    if(var=='a'){
      individual_Blink(strip.Color(  0,   0,   0, 255), LONG_DELAY_TIME);             // individual LED blink
    }
    if(var=='b'){
      individual_On(strip.Color( 0, 0, 0, 255),0);
      Serial.flush();
    }
    if(var=='c'){
      individual_On(strip.Color( 0, 0, 0, 255),1);
      Serial.flush();
    }
    if(var=='d'){
      individual_On(strip.Color( 0, 0, 0, 255),2);
      Serial.flush();
    }
    if(var=='e'){
      individual_On(strip.Color( 0, 0, 0, 255),3);
      Serial.flush();
    }
    if(var=='f'){
      individual_On(strip.Color( 0, 0, 0, 255),4);
      Serial.flush();
    }
    if(var=='g'){
      individual_On(strip.Color( 0, 0, 0, 255),5);
      Serial.flush();
    }
    if(var=='h'){
      individual_On(strip.Color( 0, 0, 0, 255),6);
      Serial.flush();
    }
    if(var=='i'){
      individual_On(strip.Color( 0, 0, 0, 255),7);
      Serial.flush();
    }
    if(var=='j'){
      individual_On(strip.Color( 0, 0, 0, 255),8);
      Serial.flush();
    }
    if(var=='k'){
      individual_On(strip.Color( 0, 0, 0, 255),9);
    }
    if(var=='l'){
      individual_On(strip.Color( 0, 0, 0, 255),10);
    }
    if(var=='m'){
      individual_On(strip.Color( 0, 0, 0, 255),11);
    }
    if(var=='n'){
      individual_On(strip.Color( 0, 0, 0, 255),12);
    }
    if(var=='o'){
      individual_On(strip.Color( 0, 0, 0, 255),13);
    }
    if(var=='p'){
      individual_On(strip.Color( 0, 0, 0, 255),14);
    }
    if(var=='q'){
      individual_On(strip.Color( 0, 0, 0, 255),15);
    }
    if(var=='r'){
      short_loop();
    } 
...
void odd_Blink(uint8_t wait) {
  for(int i=0; i<strip.numPixels(); i+=2){
    strip.setPixelColor(i,strip.Color(0, 0, 0, strip.gamma8(255)));
    strip.show();
    delay(DELAY_TIME);
  }
}
...
void short_loop(){
  for(int i=0; i<10; i++){
    half_on(strip.Color(  0,  0,  0,  255), 0, 8, DELAY_TIME);
    half_on(strip.Color(  0,  0,  0,  0), 8, 15, DELAY_TIME);
    delay(SHORT_DELAY_TIME);
    half_on(strip.Color(  0,  0,  0,  255), 8, 15, DELAY_TIME);
    half_on(strip.Color(  0,  0,  0,  0), 0, 8, DELAY_TIME); 
  }
}
